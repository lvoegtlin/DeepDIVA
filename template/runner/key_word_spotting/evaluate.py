...
def validate(val_loader, model, criterion, writer, epoch, no_cuda=False, log_interval=20, **kwargs):
    ...
    class_ids = np.zeros(len(val_loader), dtype=np.int32)
    embedding_size = val_loader.dataset.embedding_size()
    embeddings = np.zeros((len(val_loader), embedding_size), dtype=np.float32)
    outputs = np.zeros((len(val_loader), embedding_size), dtype=np.float32)

    pbar = enumerate(tqdm(val_loader, total=len(val_loader), unit='batch', ncols=150, leave=False))

    for sample_idx, (word_img, embedding, class_id, is_query) in pbar:
    ...
    logging.info('mAP: %3.2f', np.mean(ave_precs_qbe[0] + ave_precs_qbe[1]) * 100)
